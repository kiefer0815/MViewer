...
//                word.append(strLine);
   ...
   //向前翻页
    public void prePage(){
        if(currentPage==null || mBookPageWidget==null){
            Toast.makeText(mContext, "出错啦，请退出后重新上车~~", Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    public void nextPage(){
        if(mBookPageWidget==null || currentPage==null){
            Toast.makeText(mContext, "出错啦，请退出后重新上车~~", Toast.LENGTH_SHORT).show();
            return;
        }
...
//                    height +=  paragraphSpace;
    ...
    public int getCurrentCharter(){
        int num = 0;
        if(currentPage==null) return num;
