package com.flyersoft.moonreader.utils;

import android.content.ContentValues;
import android.os.Environment;
import android.text.TextUtils;

import com.flyersoft.moonreader.bean.Cache;
import com.flyersoft.moonreader.component.db.BookList;
import com.flyersoft.moonreader.component.db.BookCatalogue;
...
//            bufStr = bufStr.replaceAll(" ","");
            bufStr = bufStr.replaceAll("\u0000","");
            bufStr = bufStr.toUpperCase();
            bufStr = bufStr.replaceAll("辣文小说网","");
            bufStr = bufStr.replaceAll("WWW.LAWEN2.COM","");
            bufStr = bufStr.replaceAll("（）","");
            bufStr = bufStr.replaceAll("()","");
            bufStr =bufStr.replaceAll("免费TXT小说下载","");
            bufStr =bufStr.replaceAll("免费电子书下载","");
            bufStr =bufStr.replaceAll("电子书下载","");
            bufStr =bufStr.replaceAll("TXT小说下载","");
            bufStr =bufStr.replaceAll("好看的TXT电子书","");
            bufStr =bufStr.replaceAll("ROU","肉");
            bufStr =bufStr.replaceAll("YIN","淫");
            bufStr =bufStr.replaceAll("RU","乳");
            bufStr =bufStr.replaceAll("GUI","龟");
            bufStr =bufStr.replaceAll("BI","逼");
    ...
    public char[] block(int index) {
        ...
        if (block == null) {
            try {
                ...
                if (size < 0) {
                    throw new RuntimeException("Error during reading book size < 0" + fileName(index));
                }
                ...
                if (reader.read(block) != block.length) {
                    throw new RuntimeException("Error during reading length not equal" + fileName(index));
                }
                reader.close();
            } catch (IOException e) {
                throw new RuntimeException("Error during reading " + fileName(index) + e.getMessage());
            }
